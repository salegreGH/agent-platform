...
function App(){
  ...
  const [state, setState] = useState({agents:[], skills:[], proposals:[], tasks:[], forms:[], connectors:{}});
  ...
  const tabItems = useMemo(()=>([
    {id:"chat",label:"Chat"},{id:"tasks",label:"Tasks",count:state.tasks.length},{id:"agents",label:"Agents",count:state.agents.length},
    {id:"connectors",label:"Connectors",count:Object.keys(state.connectors||{}).length},{id:"skills",label:"Skills",count:state.skills.length},
    {id:"proposals",label:"Proposals",count:state.proposals.filter(p=>p.status==='proposed').length},{id:"forms",label:"Forms",count:state.forms.length},
