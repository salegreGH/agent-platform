...
function App(){
  ...
  async function handleAction(action, runId){
    ...
    if(action.kind === 'switch_browser'){ setMsg('no es viable'); return; }
    if(action.kind === 'retry'){
      const rid = runId || wizard?.run_id;
      if(!rid) return;
      const r = await fetch(`/core/run/${rid}/retry`,{method:'POST'});
      const j = await r.json();
      if(j.reply) setChat(c=>[...c,{role:'assistant',content:j.reply,run:j.run}]);
      await refreshState();
      return;
    }
  }
  ...
  function wizardPanel(){
    ...
    return React.createElement('div',{className:'p-3 rounded border border-amber-700 bg-amber-950/30 text-sm mb-3'},
      React.createElement('div',{className:'font-semibold mb-1'},'Outlook Login'),
      React.createElement('div',{className:'text-xs mb-2'},`Estado real: ${wizard.status_text || wizard.state}`),
  ...
  function runProgress(run){
    ...
    return React.createElement('div',{className:'text-xs text-blue-300 mt-1'},`RUNNING: ${running.action}`);
  }

  function panel(){
    if(sidebarTab==='runs') return React.createElement('div',{},(state.runs||[]).map(r=>React.createElement('div',{key:r.run_id,className:'p-2 border border-slate-800 rounded mb-2 text-xs'},
      React.createElement('div',null,`${r.run_id} · ${r.metadata?.task_state||r.status}`),
      runProgress(r),
      r.metadata?.recovery?.evidence?.screenshot_path && React.createElement('div',{className:'mt-1 text-[10px]'},`screenshot: ${r.metadata.recovery.evidence.screenshot_path}`)
    )));
    ...
    return React.createElement('div',{className:'text-xs text-slate-400'},'Escribe una petición.');
  }
  ...
  return React.createElement('div',{className:'h-screen flex'},
    React.createElement('div',{className:'w-80 p-3 border-r border-slate-800 bg-slate-900/40'},
      React.createElement('div',{className:'text-lg font-semibold mb-3'},'Agent Platform v6'),
    ...
    React.createElement('div',{className:'flex-1 flex flex-col'},
      React.createElement('div',{className:'flex-1 overflow-auto p-6 space-y-3'},wizardPanel(), chat.map(renderMessage), busy && React.createElement('div',null,'RUNNING...'), React.createElement('div',{ref:endRef})),
      React.createElement('div',{className:'p-3 border-t border-slate-800 flex gap-2'},React.createElement('textarea',{value:msg,onChange:e=>setMsg(e.target.value),className:'flex-1 h-12 bg-slate-900 border border-slate-700 rounded p-2',placeholder:'Pide el último email...'}),React.createElement('button',{onClick:send,className:'px-4 bg-blue-600 rounded'},'Enviar'))
    )
  );
}
